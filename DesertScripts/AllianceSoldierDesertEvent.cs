...
public class AllianceSoldierDesertEvent : MonoBehaviour {
	public List<AllianceClassDesert> allyNpc = new List<AllianceClassDesert>();
	private List<AllianceClassDesert> npc = new List<AllianceClassDesert>();
	...
	void Start ()
	{
		...
		mds = brumbrum.GetComponent<MissionDesertScript> ();
		brumTrans = brumbrum.GetComponent<Transform> ();
		for (int i = 0; i < allyNpc.Count; i++) {
			//for(int j = 0; j < allyNpc[i].SunnyPatrol.Length; j++){
			npc.Add(new AllianceClassDesert(allyNpc[i].allianceNpc.GetComponent<Animator>(), allyNpc[i].allianceNpc.GetComponent<NavMeshAgent>(),
	...
	private void EnableRagDoll (int i)							//wlaczenie ragdolla
	{
		if (npc [i].isLife == true) {
			...
			EnableOrDisablePartSys(i, false);
			for (int j = 0; j < npc[i].rBody.Length; j++) {
